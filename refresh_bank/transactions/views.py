...
class LoanRequestView(TransactionCreateMixin):
    ...
    def form_valid(self, form):
        amount = form.cleaned_data.get('amount')
        account = self.request.user.account
        current_loan_count = Transaction.objects.filter(account=account,transaction_type=3,loan_approve=True).count()
...
class SendMoneyView(TransactionCreateMixin):
    ...
    def form_valid(self, form):
        ...
        receive_account = UserBankAccount.objects.get(account_no = to_account)
